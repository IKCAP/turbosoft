...
DataViewer.prototype.openDataTypeEditor = function(args) {
    ...
    var uploadfilesbtn = {
            ...
            iconCls: 'icon-upload fa fa-browngrey',
            handler: function() {
                var win = new Ext.Window({
				    ...
				    items : [{
						...
						uploadButtonCls : 'icon-upload fa fa-browngrey',
						cancelButtonCls : 'icon-cancel fa fa-red',
                        listeners : {
    ...
    var gridPanel = new Ext.grid.GridPanel({
        ...
        autoHeight: true,
        plugins: plugins,
        title: 'Metadata Properties for ' + name + ' Files',
        columns: [{
            dataIndex: 'prop',
            flex: 1,
            header: 'Property',
    ...
    if (inhProperties.length) {
        ...
        var inhGridPanel = new Ext.grid.GridPanel({
            title: 'Inherited Metadata Properties for ' + name + ' Files',
            columns: [{
                dataIndex: 'prop',
                header: 'Property',
                flex: 1,
...
DataViewer.prototype.handleTreeClick = function(view, rec, item, ind, event) {
    ...
    var tab = this.openNewIconTab(tabName, (rec.data.isClass ? 
    		(rec.raw.readonly ? 'icon-folder-open fa-title fa-grey' 
...
DataViewer.prototype.createMetricsTreePanel = function(metricsHierarchy) {
    ...
    this.metricsTreePanel = new Ext.tree.TreePanel({
        ...
        autoScroll: true,
        title: 'Metrics',
        iconCls: 'icon-file-alt fa fa-blue',
        // TEMPORARY FIXME
        hidden: true,
        containerScroll: true,
...
DataViewer.prototype.getTree = function(data) {
    ...
    var treenode = {
        ...
        iconCls: (item.type == 1 
        		? (item.readonly ? 'icon-folder fa fa-grey' 
        				: 'icon-folder fa fa-yellow' ) 
        		: 'icon-file-alt fa fa-blue'),
        expIconCls: (item.type == 1 
                ? (item.readonly ? 'icon-folder-open fa fa-grey' 
...
DataViewer.prototype.addDataForType = function(parentNode) {
    var This = this;
    if (!parentNode) {
        showError('Please select a File Type from below to add data for');
        return;
    }
    if (parentNode.data.type == 2) {
        showError('Please select a File Type from below to add data for. You have current selected Data');
        return;
    }
...
DataViewer.prototype.addDatatype = function(parentNode) {
    ...
    Ext.Msg.prompt("Add File Type", "Enter name for the new File Type:", function(btn, text) {
        if (btn == 'ok' && text) {
            var newid = This.ontns + getRDFID(text);
            var enode = This.dataTreePanel.getStore().getNodeById(newid);
            if (enode) {
                showError('File Type ' + text + ' already exists');
                return;
            }
